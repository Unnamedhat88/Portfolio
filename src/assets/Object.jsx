...
export default function Object({positionofxz,setpositionofxz,cameraBusy,focusObject, setFocusObject,activeDiv, zoomedin, setZoomedin,tutorial, setTutorial}){
    ...
    <primitive scale={0.5} object={gltf.scene} onClick={(e)=>{
      ...
      {/* for TV */}
      <group position={[6.1243, 6.309, 10.135]} rotation={[0, 1.27, 0]} scale={!isMobile?[0.959, 0.834, 0.959]:[0.999, 0.834, 0.959]} >
        {(zoomedin&&activeDiv==0)&&<Html distanceFactor={!isMobile?1.0:1.0} transform center position={isMobile?[-0.8,-3.45,2.29]:[-0.9,-3.5,2.29]} scale={[3,3.4,3]} style={{transition:"opacity 200ms",opacity:(zoomedin&&activeDiv==0)?"1":"0" }} >
      ...
      <group  position={[-62.901+0.206, 9.075, -56.713+0.030]} rotation={[1.508+0.035, 0.163-0.088, -1.199-0.001]} scale={[3.413+0.05, 1.0, 2.526+0.05]}>
        ...
        {(zoomedin&&activeDiv==2)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==2?1:0, pointerEvents:"auto"}} distanceFactor={!isMobile?1.0:0.95} transform position={[0.002,-2,-0.04]} scale={[1.15,1.51,1.15]} rotation={[-(Math.PI)/2,0,0]}>
    ...
    <group position={[-88.15, 5.86, -90.755]} scale={[1.866, 1.153, 1.866]} >
      ...
      {(zoomedin&&activeDiv==3)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==3?1:0}} distanceFactor={1.0} transform position={[1.73,-1.76,2.2]} scale={[1.2,1.9,1.5]} rotation={[Math.PI-0.050,-Math.PI-0.61,-Math.PI+0.03]}>
