...
export default function Object({positionofxz,setpositionofxz,cameraBusy,focusObject, setFocusObject,activeDiv, zoomedin, setZoomedin,tutorial, setTutorial}){
    ...
    <primitive scale={0.5} object={gltf.scene} onClick={(e)=>{
      ...
      <group  position={[-62.901+0.106, 9.075, -56.713+0.030]} rotation={[1.508+0.035, 0.163-0.088, -1.199-0.001]} scale={[3.413+0.05, 0.085+0.05, 2.526+0.05]}>
        {(zoomedin&&activeDiv==2)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==2?1:0}} distanceFactor={!isMobile?1.0:1.2} transform position={[8.57,6.4,13.4]} scale={[4,6.3,4]} rotation={[0,0.577,0]}>
          <div style={{width:size3.x*300, height:size3.y*170}} className="">
              <Projects animateCamera={animateCamera} cameraBusy={cameraBusy} originalCameraPosition={originalCameraPosition} originalLookAt={originalLookAt} camera={camera} positionofxz={positionofxz} setZoomedin={setZoomedin}  zoomedin={zoomedin}></Projects>
          </div>
        ...
        {/* {(zoomedin&&activeDiv==2)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==2?1:0}} distanceFactor={!isMobile?1.0:1.2} transform position={[0.002,-2,-0.04]} scale={[1,1.31,1]} rotation={[(Math.PI)/2,-Math.PI,-Math.PI]}>
