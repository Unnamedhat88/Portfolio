...
export default function Object({positionofxz,setpositionofxz,cameraBusy,focusObject, setFocusObject,activeDiv, zoomedin, setZoomedin,tutorial, setTutorial}){
    ...
    <primitive scale={0.5} object={gltf.scene} onClick={(e)=>{
      ...
      <group  position={[-62.901+0.206, 9.075, -56.713+0.10]} rotation={[1.508+0.035, 0.163-0.088, -1.199-0.001]} scale={[3.413+0.05, 1.0, 2.526+0.05]}>
        ...
        {(zoomedin&&activeDiv==2)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==2?1:0, pointerEvents:"auto"}} distanceFactor={!isMobile?1.01:1.03} transform position={isMobile?[-0.002,0,-0.08]:[0.002,0,-0.04]} scale={[1.15,1.51,1.15]} rotation={[-(Math.PI)/2,0,0]}>
    ...
    <group position={[-88.15, 5.86, -90.755]} scale={[1.866, 1.153, 1.866]} >
      ...
      {(zoomedin&&activeDiv==3)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==3?1:0}} distanceFactor={isMobile?1.015:1.0} transform position={isMobile?[1.73,-2.05,2.285]:[1.73,-1.76,2.2]} scale={[1.2,1.9,1.5]} rotation={[Math.PI-0.050,-Math.PI-0.61,-Math.PI+0.03]}>
