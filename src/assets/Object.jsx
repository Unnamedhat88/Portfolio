...
export default function Object({positionofxz,setpositionofxz,cameraBusy,focusObject, setFocusObject,activeDiv, zoomedin, setZoomedin,tutorial, setTutorial}){
    ...
    <primitive scale={0.5} object={gltf.scene} onClick={(e)=>{
      ...
      <group position={[6.1243, 6.309, 10.135]} rotation={[0, 1.27, 0]} scale={[0.959, 0.834, 0.959]} >
        {(zoomedin&&activeDiv==0)&&<Html distanceFactor={!isMobile?1.0:1.2} transform center position={!isMobile?[-0.8,-3.5,2.29]:[-0.1,-4.1,2.29]} scale={[3,3.4,3]} style={{transition:"opacity 200ms",opacity:(zoomedin&&activeDiv==0)?"1":"0" }} >
      ...
      <group position={[-33.589, 5.86, -41.283]} rotation={[0, 0.095, 0]} scale={[1.23, 0.76, 1.23]}>
          ...
          {(zoomedin&&activeDiv==1)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==1?1:0}} distanceFactor={!isMobile?1.0:1.2} transform position={[8.57,6.4,13.4]} scale={[4,6.3,4]} rotation={[0,0.577,0]}>
      ...
      <group  position={[-62.901+0.106, 9.075, -56.713+0.030]} rotation={[1.508+0.035, 0.163-0.088, -1.199-0.001]} scale={[3.413+0.05, 1.0, 2.526+0.05]}>
       
        {(zoomedin&&activeDiv==2)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==2?1:0, pointerEvents:"auto"}} distanceFactor={!isMobile?1.0:1.2} transform position={[0.002,-2,-0.04]} scale={[1,1.31,1]} rotation={[-(Math.PI)/2,0,0]}>
          <div style={{width:size2.x*172, height:size2.y*40,pointerEvents:"auto"}} className="">
    ...
    <group position={[-88.15, 5.86, -90.755]} scale={[1.866, 1.153, 1.866]} >
      ...
      {(zoomedin&&activeDiv==3)&&<Html center style={{ transition:"opacity 1.4s", opacity: activeDiv==3?1:0}} distanceFactor={!isMobile?1.0:1.2} transform position={[1.73,-1.76,2.2]} scale={[1.2,1.9,1.5]} rotation={[Math.PI-0.050,-Math.PI-0.61,-Math.PI+0.03]}>
